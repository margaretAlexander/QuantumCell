...
organ = input dataset name
min_metacell_size = smallest metacell size to run metacell with
max_metacell_size = largest metacell size to run metacell with
interval = interval between min_metacell_size and max_metacell_size to run metacell with
filter_later = wehter to filter lateral genes (exclude this argument if not filtering)
...
`python get_cell_membership.py organ --filter_lateral`
